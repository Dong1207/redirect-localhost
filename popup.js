...
class RuleManager {
  ...
  testUrl(url) {
    ...
    this.rules.forEach((rule, index) => {
      ...
      if (
        rule.disabled ||
        (rule.section && !this.isSectionEnabled(rule.section))
      )
        return;
...
class PopupUI {
  ...
  isValidRedirectUrl(url) {
    return URLUtils.isValidRedirectUrl(url);
  }
  ...
  truncateUrl(url, maxLength) {
    ...
    // Use URLUtils for standard truncation
    return URLUtils.truncateUrl(url, maxLength);
  }
  ...
  loadRedirectHistory() {
    chrome.storage.local.get(["redirectHistory"], (result) => {
      ...
      if (history.length === 0) {
        this.elements.redirectHistory.appendChild(
          DOM.createElement(
            "div",
      ...
      history.forEach((item) => {
        const fromUrl = URLUtils.truncateUrl(item.fromUrl, 60);
        const toUrl = URLUtils.truncateUrl(item.toUrl, 40);
        ...
        const historyItem = DOM.createElement("div", {
          className:
            item.toUrl === "Unknown destination" || !item.toUrl
              ? "history__item history__item--incomplete"
              : "history__item",
        });

        // Add content using DOM utility
        historyItem.appendChild(
          DOM.createElement("div", {className: "history__from"}, [
            DOM.createElement("strong", {}, "From:"),
            document.createTextNode(" " + fromUrl),
          ])
        );
  ...
  displayRulesForSection(sectionName, container) {
    ...
    const rulesForSection = this.ruleManager.rules.filter(
      (rule, index) => rule.section === sectionName
    );
    // Display each rule
  ...
  toggleDonateView(isVisible) {
    ...
    console.log(
      "addSectionBtn before:",
      addSectionBtn.classList.contains("hidden") ? "hidden" : "visible"
    );
